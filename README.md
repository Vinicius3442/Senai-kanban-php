...
4. Importe o arquivo `database/script.sql` fornecido neste repositório.
...
1. Acesse o projeto no navegador: `http://localhost:[porta]/`
...
4. **Gerenciamento (Quadro Kanban):** Na aba "Kanban Board", você visualizará o quadro com as 3 colunas:
   ...
   Você pode **Editar** (abre um modal para editar os dados), **Excluir** a tarefa, ou movê-la entre os status clicando nos **ícones de setas** presentes no rodapé do card da tarefa.

## Entregas Atendidas

1. **DER:** (Você precisa salvar a imagem na pasta `docs/` conforme o PDF)
2. **Banco de Dados:** Localizado em `database/script.sql`
3. **Caso de Uso:** (Você precisa salvar a imagem na pasta `docs/` conforme o PDF)
...
- **Lucide Icons** via CDN para os ícones vetoriais profissionais.
- As fontes **Inter** e **Caveat** (estilo manuscrito) são carregadas via Google Fonts.
